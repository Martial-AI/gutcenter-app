...
class StudentController extends Controller
{
    ...
    public function index(Request $request): View
    {
        ...
        $totalStudents = $isTeacher ? null : Student::whereHas('enrollments', fn ($query) => $query->where('status', 'active')->whereHas('schoolClass', fn ($q) => $q->whereRaw("LOWER(name) NOT LIKE '%ecolage%' AND LOWER(name) NOT LIKE '%écolage%'")))->count();
    ...
    public function exportList(Request $request): \Illuminate\Http\Response
    {
        $this->authorize('viewAny', Student::class);
        ...
        $studentsQuery = Student::with(['enrollments' => fn ($q) => $q->where('status', 'active')->with('schoolClass')])
            ->whereHas('enrollments', function ($q) use ($classId) {
                $q->where('status', 'active')
                  ->whereHas('schoolClass', function ($sc) use ($classId) {
                      $sc->whereRaw("LOWER(name) NOT LIKE '%ecolage%' AND LOWER(name) NOT LIKE '%écolage%'");
                      if ($classId) {
                          $sc->where('id', $classId);
                      }
                  });
        ...
        // Group by class name, sorted
        $grouped = $students
            ->sortBy(fn ($s) => $s->enrollments->first()?->schoolClass?->name ?? 'zzz')
            ->groupBy(fn ($s) => $s->enrollments->first()?->schoolClass?->name ?? '—');

        $totalStudents = $students->count();
        $totalMale    = $students->where('gender', 'male')->count();
        $totalFemale  = $students->where('gender', 'female')->count();

        // Current active academic year label
        $academicYearQuery = \App\Models\SchoolClass::with('academicYear')
            ->whereRaw("LOWER(name) NOT LIKE '%ecolage%' AND LOWER(name) NOT LIKE '%écolage%'")
            ->whereHas('enrollments', fn ($q) => $q->where('status', 'active'));

        if ($classId) {
            $academicYearQuery->where('id', $classId);
        }

        $academicYear = $academicYearQuery
            ->get()
            ->pluck('academicYear.name')
            ->filter()
            ->unique()
            ->join(' / ');

        $logoPath = public_path('images/gut-logo.png');
        $logo = file_exists($logoPath) ? 'data:image/png;base64,' . base64_encode((string) file_get_contents($logoPath)) : null;

        $pdf = Pdf::loadView('students.export-pdf', compact(
            'grouped', 'totalStudents', 'totalMale', 'totalFemale', 'academicYear', 'logo'
        ))->setPaper('a4', 'portrait');

        // Build filename: include class name if filtering by single class
        $className = $classId ? \App\Models\SchoolClass::find($classId)?->name : null;
        $slug = $className ? \Illuminate\Support\Str::slug($className) : 'tous';
        $filename = 'liste-eleves-' . $slug . '-' . now()->format('Y-m-d') . '.pdf';
    ...
    public function store(StoreStudentRequest $request, StudentIdentifierService $identifiers, RegistrationFeeService $registrationFees, MonthlyFeeReceiptService $receipts): RedirectResponse
    {
        ...
        return to_route('students.index', ['enrolled_receipt' => $student->id])
            ->with('success', __('Student enrolled successfully.'))
            ->with('enrollment_receipt', [
