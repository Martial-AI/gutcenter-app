...
<nav x-data="{ open: false }" class="sticky top-0 z-40 border-b border-emerald-100/80 bg-white/90 shadow-sm backdrop-blur">
    ...
    <div :class="{'block': open, 'hidden': ! open}" class="hidden sm:hidden border-b border-slate-200 dark:border-slate-800 bg-white/95 dark:bg-slate-900/95 backdrop-blur-xl">
        <div class="space-y-1 pb-3 pt-2">
            ...
            @can('attendance.view')<x-responsive-nav-link :href="route('attendance.index')" :active="request()->routeIs('attendance.*')">{{ __('Attendance') }}</x-responsive-nav-link>@endcan @can('statistics.view')<x-responsive-nav-link :href="route('attendance.index')" :active="request()->routeIs('attendance.*') || request()->routeIs('statistics.*')">{{ __('Statistics') }}</x-responsive-nav-link>@endcan 
            @can('roles.manage')
