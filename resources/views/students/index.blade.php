...
<script>
document.addEventListener('DOMContentLoaded',()=>{document.querySelectorAll('#student-class-filters a').forEach(link=>{const label=link.textContent.normalize('NFD').replace(/[\u0300-\u036f]/g,'').toLowerCase();if(label.includes('ecolage') || link.href.includes('school-fees'))link.remove()})});
</script>
<style>
...
<div id="student-export-modal" class="fixed inset-0 z-[9999] hidden items-center justify-center bg-black/60 p-4"><div class="w-full max-w-sm rounded-2xl bg-white p-6 text-center shadow-2xl"><div class="mx-auto mb-3 flex h-12 w-12 items-center justify-center rounded-full bg-emerald-100 text-emerald-700"><svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 10v6m0 0l-3-3m3 3l3-3m2 8H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"/></svg></div><h3 class="text-lg font-semibold text-gray-900">{{ __('Download student list') }}</h3><p id="student-export-message" class="mt-2 text-sm text-gray-600">{{ __('Download all enrolled students grouped by class as a PDF?') }}</p><div class="mt-6 flex justify-center gap-3"><button type="button" onclick="closeStudentExportModal()" class="rounded-lg border px-4 py-2">{{ __('Cancel') }}</button><button type="button" onclick="confirmStudentExport()" class="rounded-lg bg-emerald-700 px-4 py-2 text-white">{{ __('Download PDF') }}</button></div></div></div>
<script>
let studentExportTimer=null,studentExportClassId=null;
const exportBaseUrl=@json(route('students.export-list'));
function openStudentExportModal(classId,className){
    studentExportClassId=classId||null;
    const msg=document.getElementById('student-export-message');
    if(className){msg.textContent=@json(__('Download the student list for'))+ ' « '+className+' » ?';}
    else{msg.textContent=@json(__('Download all enrolled students grouped by class as a PDF?'));}
    const modal=document.getElementById('student-export-modal');modal.classList.remove('hidden');modal.classList.add('flex');
}
function closeStudentExportModal(){const modal=document.getElementById('student-export-modal');modal.classList.add('hidden');modal.classList.remove('flex');studentExportClassId=null;}
function confirmStudentExport(){let url=exportBaseUrl;if(studentExportClassId)url+='?class_id='+studentExportClassId;window.location.href=url;closeStudentExportModal();}
document.addEventListener('DOMContentLoaded',()=>{
    // Long press on "Total students" button (all classes)
    const button=document.getElementById('total-students-download');
    if(button){const start=event=>{event.preventDefault();studentExportTimer=setTimeout(()=>openStudentExportModal(null,null),650)};const stop=()=>{clearTimeout(studentExportTimer);studentExportTimer=null};button.addEventListener('pointerdown',start);['pointerup','pointerleave','pointercancel'].forEach(name=>button.addEventListener(name,stop));button.addEventListener('contextmenu',event=>{event.preventDefault();openStudentExportModal(null,null)})}
    // Long press on each class filter button
    document.querySelectorAll('#student-class-filters a').forEach(link=>{
        const url=new URL(link.href,location.origin);
        const classId=url.searchParams.get('class_id');
        if(!classId)return; // skip "All classes"
        const className=link.textContent.replace(/\s*\(\d+\)\s*$/,'').trim();
        let timer=null,longPressed=false;
        link.addEventListener('pointerdown',()=>{longPressed=false;timer=setTimeout(()=>{longPressed=true;openStudentExportModal(classId,className)},650)});
        const stop=()=>{clearTimeout(timer);timer=null};
        ['pointerup','pointerleave','pointercancel'].forEach(name=>link.addEventListener(name,stop));
        link.addEventListener('contextmenu',e=>{e.preventDefault();longPressed=true;openStudentExportModal(classId,className)});
        link.addEventListener('click',e=>{if(longPressed){e.preventDefault();longPressed=false}});
    });
});
</script>
...
document.getElementById('enrollment-frame')?.addEventListener('load',function(){try{const frameUrl=new URL(this.contentWindow.location.href);if(frameUrl.pathname.endsWith('/students')||frameUrl.pathname.endsWith('/students/')){const receiptId=frameUrl.searchParams.get('enrolled_receipt');closeEnrollmentModal();if(receiptId){window.location.href=frameUrl.href;}else{window.location.reload();}}}catch(error){}});
</script>
@php
    $receiptStudentId = request('enrolled_receipt');
    $receiptStudent = $receiptStudentId ? \App\Models\Student::find($receiptStudentId) : null;
    $hasEnrollmentReceipt = session('enrollment_receipt') || ($receiptStudent && auth()->user()->can('view', $receiptStudent));
    $enrollmentReceiptPreview = session('enrollment_receipt.preview') ?? ($receiptStudent ? route('students.enrollment-receipt-preview', $receiptStudent) : null);
    $enrollmentReceiptPdf = session('enrollment_receipt.pdf') ?? ($receiptStudent ? route('students.enrollment-receipt', $receiptStudent) : null);
@endphp
@if($hasEnrollmentReceipt && $enrollmentReceiptPreview && $enrollmentReceiptPdf)
<div id="enrollment-receipt-modal" class="fixed inset-0 flex items-center justify-center bg-black/60 p-4" style="z-index:999999"><div class="flex h-[82vh] w-full max-w-sm flex-col overflow-hidden rounded-2xl bg-white shadow-2xl"><div class="flex items-center justify-between border-b px-4 py-3 bg-gray-50"><h3 class="font-semibold text-gray-800">{{ __('Enrollment receipt') }}</h3><button type="button" onclick="closeEnrollmentReceipt()" class="rounded-lg px-2.5 py-1 text-sm font-medium text-gray-500 hover:bg-gray-200 hover:text-gray-800 transition" aria-label="{{ __('Close') }}">×</button></div><iframe class="min-h-0 flex-1 w-full bg-white" src="{{ $enrollmentReceiptPreview }}" title="{{ __('Enrollment receipt') }}"></iframe><div class="flex justify-end gap-3 border-t bg-gray-50 p-3"><button type="button" onclick="printEnrollmentReceipt()" class="rounded-lg border border-gray-300 bg-white px-3 py-2 text-sm font-medium text-gray-700 hover:bg-gray-50 transition shadow-sm">{{ __('Print') }}</button><a href="{{ $enrollmentReceiptPdf }}?download=1" class="rounded-lg bg-emerald-700 px-4 py-2 text-sm font-medium text-white hover:bg-emerald-800 transition shadow-sm">{{ __('Download') }}</a></div></div></div><iframe id="enrollment-print-frame" class="hidden"></iframe><script>function closeEnrollmentReceipt(){const m=document.getElementById('enrollment-receipt-modal');if(m)m.remove();const u=new URL(window.location.href);if(u.searchParams.has('enrolled_receipt')){u.searchParams.delete('enrolled_receipt');window.history.replaceState({},document.title,u.pathname+(u.search?u.search:''));}}function printEnrollmentReceipt(){const f=document.getElementById('enrollment-print-frame');f.onload=()=>setTimeout(()=>{f.contentWindow?.focus();f.contentWindow?.print();},300);f.src='{{ $enrollmentReceiptPdf }}';}</script>
@endif
<style>
