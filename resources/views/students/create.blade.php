...
<form method="POST" enctype="multipart/form-data" action="{{ route('students.store') }}" class="space-y-5 rounded-xl bg-white p-6 shadow">@csrf
    ...
    <section id="enrollment-payment" class="hidden rounded-xl border border-emerald-100 bg-emerald-50 p-4">
        ...
        <div id="registration-payment" class="mt-3 hidden"><p id="registration-total" class="text-sm font-medium"></p><label class="mt-3 flex items-center gap-2"><input id="registration-full" type="checkbox" name="registration_payment" value="full"> {{ __('Pay the registration fee in full') }}</label><div class="mt-3 flex flex-wrap items-end gap-3"><label class="text-sm">{{ __('Or remaining amount') }}<input id="registration-remaining" name="registration_remaining_amount" type="number" min="0" step="0.01" placeholder="ex: 50 000 Ar" class="mt-1 block rounded border-gray-300"></label><label class="text-sm">{{ __('Remaining balance deadline (optional)') }}<input id="registration-due-date" name="registration_due_date" type="date" class="mt-1 block rounded border-gray-300"></label></div><input id="registration-payment-mode" type="hidden" name="registration_payment" value="none"></div>
    ...
    <div class="grid gap-4 md:grid-cols-2"><input name="guardian[first_name]" required placeholder="{{ __('First name') }}" class="rounded border-gray-300"><input name="guardian[last_name]" required placeholder="{{ __('Last name') }}" class="rounded border-gray-300"><input name="guardian[phone]" required placeholder="{{ __('Phone') }}" class="rounded border-gray-300"><input name="guardian[email]" type="email" placeholder="{{ __('Email') }} ({{ __('optional') }})" class="rounded border-gray-300"></div>
